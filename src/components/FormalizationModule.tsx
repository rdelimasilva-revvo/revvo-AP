...
import { getTooltipText } from '../data/glossary';
import {
  CheckCircle,
  Clock,
  XCircle,
...
export const FormalizationModule: React.FC<FormalizationModuleProps> = ({ clients }) => {
  ...
  const stats = {
    ...
    optInApproved: clientsData.filter(d => d.optInStatus === 'approved').length,
    contractsFormalized: clientsData.filter(d => d.contractStatus === 'formalized').length,
    active: clientsData.filter(d => d.businessFlow === 'active').length,
    totalTargetValue: clientsData.reduce((sum, d) => sum + d.targetValue, 0),
    totalCurrentValue: clientsData.reduce((sum, d) => sum + d.currentValue, 0)
  };
