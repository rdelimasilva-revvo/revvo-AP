...
import {
  ...
  Search, CheckCircle, XCircle, Plus, X, Trash2
} from 'lucide-react';
...
export const DisputeAutomationModule: React.FC = () => {
  ...
  const handleCreateRule = () => {
    ...
    const rule: AutomationRule = {
      ...
      autoAssignTo: newRule.autoAssignTo,
      thresholdPercent: newRule.thresholdPercent,
      createdAt: new Date(),
      lastTriggered: null,
      triggerCount: 0,
    };
    ...
    setNewRule({ name: '', description: '', priority: 'medium', autoAssignTo: '', thresholdPercent: 0 });
    setHasChanges(true);
    showToast('success', 'Nova regra criada com sucesso');
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-gray-900">Regras de automação ({rules.length})</h3>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {rules.map(rule => {
          ...
          const isDefault = rule.id === 'rule-blocked-reduction' || rule.id === 'rule-settled-less';
          const iconBg = rule.id === 'rule-blocked-reduction' ? 'bg-orange-100 text-orange-600'
            : rule.id === 'rule-settled-less' ? 'bg-red-100 text-red-600'
            : 'bg-blue-100 text-blue-600';
          const icon = rule.id === 'rule-blocked-reduction'
            ? <ShieldAlert className="w-5 h-5" />
            : rule.id === 'rule-settled-less'
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4 overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b border-gray-200">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-lg font-semibold text-gray-900">Novo Setup de Contestação</h3>
              </div>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Ex: URs com chargeback acima de 10%"
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Descreva quando essa regra deve disparar uma contestação automática..."
                  rows={3}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setNewRule(prev => ({ ...prev, priority: e.target.value as RulePriority }))}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                  >
                    {Object.entries(PRIORITY_LABELS).map(([key, label]) => (
                      <option key={key} value={key}>{label}</option>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Tolerância mínima</label>
                  <div className="relative">
                    <input
                      type="number"
                      min={0}
                      max={100}
                      step={0.5}
                      value={newRule.thresholdPercent}
                      onChange={(e) => setNewRule(prev => ({ ...prev, thresholdPercent: parseFloat(e.target.value) || 0 }))}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Atribuir contestação a</label>
                <select
                  value={newRule.autoAssignTo}
                  onChange={(e) => setNewRule(prev => ({ ...prev, autoAssignTo: e.target.value }))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                >
                  {ASSIGNEE_OPTIONS.map(opt => (
                    <option key={opt.value} value={opt.value}>{opt.label}</option>
            ...
            <div className="flex justify-end gap-3 p-6 border-t border-gray-200 bg-gray-50">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed"
              >
                <Plus className="w-4 h-4" />
                Criar Setup
              </button>
