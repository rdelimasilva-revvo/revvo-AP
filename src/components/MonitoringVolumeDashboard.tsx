...
export const MonitoringVolumeDashboard: React.FC<MonitoringVolumeDashboardProps> = ({ onNavigate }) => {
  ...
  const notSettled = liquidationProblems.filter((p) => p.status === 'not_settled');
  const partial = liquidationProblems.filter((p) => p.status === 'partial');
  ...
  const chargebackReceivables = receivables.filter(
  ...
  const totalUrCount = receivables.length;
  const totalUrValue = receivables.reduce((sum, r) => sum + r.originalValue, 0);
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200">
        <div className="p-4 border-b border-gray-200">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-gray-900">Visão de volume</h2>
              <p className="text-xs text-gray-600">Chargeback, problemas de liquidação e liquidação parcial</p>
            </div>
          </div>
        </div>
          ...
          <div className="mb-4 p-4 rounded-lg border border-green-200 bg-green-50 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-green-700">Total de unidades de recebível no sistema</p>
              </div>
            </div>
