...
export const CreditRecoveryJourney: React.FC<CreditRecoveryJourneyProps> = ({ isOpen, onClose }) => {
  ...
  const getAutomationConfig = (clientId: string): AutomationConfig => {
    return { ...defaultAutomationConfig, ...automationConfigs.get(clientId) };
  };

  const updateAutomationConfig = (clientId: string, updates: Partial<AutomationConfig>) => {
  ...
  const handleActivateAll = () => {
    const allValid = clientsWithOptIn.every(c => {
      const config = getAutomationConfig(c.id);
      return config.targetVolume && parseFloat(config.targetVolume) > 0;
    });
    if (allValid) {
  ...
  const renderClientsStep = () => (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">Adicione os clientes e confirme a autorização de opt-in para cada um</p>
      </div>
                ...
                <div key={client.id} className="space-y-2">
                  ...
                  >
                    <div className="text-left flex-1">
                      ...
                      {isReady && (
                        <p className="text-xs text-blue-600 font-medium mt-1">Opt-in e anuência confirmados</p>
                      )}
                      {hasOptIn && !hasConsent && (
                    ...
                    <CheckCircle className={`w-5 h-5 ${isReady ? 'text-blue-600' : 'text-gray-300'}`} />
                  </div>
  ...
  const renderAutomationStep = () => (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xl font-semibold text-gray-900 mb-2">Passo 2: Configurar Automação de Recuperação</h3>
          <p className="text-gray-600">Defina o volume-alvo de busca e as configurações para cada cliente</p>
        </div>
        <div className="flex items-center gap-3">
          <button
          ...
          {clientsWithOptIn.every(c => {
            ...
            >
              <Zap className="w-4 h-4" />
              <span>Ativar Automações ({clientsWithOptIn.length})</span>
            </button>
          )}
        </div>
      </div>
            ...
            <div key={client.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              <div className="p-5 border-b border-gray-100">
                <div className="flex items-center justify-between">
                  ...
                  {config.targetVolume && parseFloat(config.targetVolume) > 0 && (
              ...
              <div className="p-5 space-y-4">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        updateAutomationConfig(client.id, { targetVolume: val });
                      }}
                      placeholder="0,00"
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button
                      onClick={() => updateAutomationConfig(client.id, { captureStrategy: 'fast' })}
                      className={`p-3 border-2 rounded-lg text-left transition-all ${
                        config.captureStrategy === 'fast'
                          ? 'border-blue-600 bg-blue-50'
                          : 'border-gray-300 hover:border-blue-300'
                      }`}
                    >
                      <div className="font-semibold text-gray-900 text-sm">Rápida</div>
                      <div className="text-xs text-gray-600">Captura o valor alvo o mais rápido possível</div>
                    </button>
                    <button
                      onClick={() => updateAutomationConfig(client.id, { captureStrategy: 'diluted' })}
                      className={`p-3 border-2 rounded-lg text-left transition-all ${
                        config.captureStrategy === 'diluted'
                          ? 'border-blue-600 bg-blue-50'
                          : 'border-gray-300 hover:border-blue-300'
                      }`}
                    >
                      <div className="font-semibold text-gray-900 text-sm">Diluída</div>
                      <div className="text-xs text-gray-600">Distribui as capturas ao longo do período</div>
                    </button>
                  </div>
                </div>

                {/* Janela de tempo - só aparece na estratégia diluída */}
                {config.captureStrategy === 'diluted' && (
                  <div className="p-4 border border-blue-200 rounded-lg bg-blue-50/50 space-y-3">
                    <p className="text-sm font-medium text-gray-700">Janela de tempo para captura diluída</p>
                ...
                {/* Pausar ao atingir alvo */}
                <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div>
                    <div className="font-medium text-gray-900 text-sm">Pausar ao atingir volume-alvo</div>
                    <div className="text-xs text-gray-600">Interrompe capturas automáticas quando o objetivo for alcançado</div>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      type="checkbox"
                      checked={config.pauseOnTargetReached}
                      onChange={(e) => updateAutomationConfig(client.id, { pauseOnTargetReached: e.target.checked })}
  ...
  const renderSummaryStep = () => {
    const totalVolume = clientsWithOptIn.reduce((sum, c) => {
      const config = getAutomationConfig(c.id);
      return sum + (parseFloat(config.targetVolume.replace(',', '.')) || 0);
    }, 0);

    return (
      <div className="space-y-6">
        <div>
          <h3 className="text-xl font-semibold text-gray-900 mb-2">Passo 3: Resumo da Recuperação</h3>
          <p className="text-gray-600">Automações de recuperação de crédito ativadas com sucesso</p>
        </div>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
          <div className="flex items-center space-x-3 mb-6">
            ...
            <div>
              <h4 className="text-lg font-semibold text-blue-900">Automações Ativadas</h4>
              <p className="text-sm text-blue-700">As automações de busca foram configuradas e estão ativas</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-4 mb-6">
            ...
            <div className="bg-white rounded-lg p-4 border border-blue-200">
              <p className="text-sm text-gray-600 mb-1">Volume-alvo Total</p>
          ...
          <div className="space-y-2">
            <h5 className="text-sm font-semibold text-gray-900 mb-3">Clientes com Automação:</h5>
            {clientsWithOptIn.map((client) => {
              const config = getAutomationConfig(client.id);
              return (
                  ...
                  <div className="flex items-center space-x-3">
                    <div className="text-right">
                      <p className="text-sm font-semibold text-blue-600">
                        R$ {config.targetVolume || '0'}
                      </p>
                      <p className="text-xs text-gray-500">
                        {config.captureStrategy === 'fast' ? 'Captura rápida' : 'Captura diluída'}
                      </p>
                    </div>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              ...
              <ul className="text-sm text-blue-700 space-y-1">
                ...
                <li>• As automações irão buscar recebíveis disponíveis até atingir o volume-alvo</li>
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded-xl shadow-xl w-full max-w-7xl max-h-[90vh] overflow-y-auto">
          <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between z-10">
            <div>
              <h2 className="text-2xl font-bold text-gray-900">Recuperação de Crédito</h2>
              <div className="flex items-center space-x-2 mt-2">
                <div className={`flex items-center space-x-2 ${currentStep === 'clients' ? 'text-blue-600' : 'text-gray-400'}`}>
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium ${currentStep === 'clients' ? 'bg-blue-600 text-white' : currentStep === 'automation' || currentStep === 'summary' ? 'bg-blue-200 text-blue-700' : 'bg-gray-300 text-gray-600'}`}>
                    1
                  </div>
                  <span className="text-sm font-medium">Clientes & Opt-in</span>
                </div>
                <div className="w-8 h-0.5 bg-gray-300"></div>
                <div className={`flex items-center space-x-2 ${currentStep === 'automation' ? 'text-blue-600' : 'text-gray-400'}`}>
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium ${currentStep === 'automation' ? 'bg-blue-600 text-white' : currentStep === 'summary' ? 'bg-blue-200 text-blue-700' : 'bg-gray-300 text-gray-600'}`}>
                    2
                  </div>
                ...
                <div className={`flex items-center space-x-2 ${currentStep === 'summary' ? 'text-blue-600' : 'text-gray-400'}`}>
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium ${currentStep === 'summary' ? 'bg-blue-600 text-white' : 'bg-gray-300 text-gray-600'}`}>
                    3
                  </div>
