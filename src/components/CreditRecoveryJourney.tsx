...
export const CreditRecoveryJourney: React.FC<CreditRecoveryJourneyProps> = ({ isOpen, onClose }) => {
  ...
  const [currentConfigClientId, setCurrentConfigClientId] = useState<string | null>(null);
  ...
  const clientsReceivables = useMemo(() => {
    return importedClients.map(client => {
      const total = Math.floor(Math.random() * 300000) + 50000;
      const locked = Math.floor(total * (Math.random() * 0.5));
      const available = total - locked;

      const acquirers = ['Cielo', 'Rede', 'Stone', 'GetNet'];
      const receivablesByAcquirer = acquirers.map(acquirer => {
        const acquirerTotal = Math.floor(total / acquirers.length);
        const acquirerLocked = Math.floor(locked / acquirers.length);
        return {
          acquirer,
          total: acquirerTotal,
          locked: acquirerLocked,
          available: acquirerTotal - acquirerLocked
        };
      });
      ...
      return {
        ...
        lockedReceivables: locked,
        availableReceivables: available,
        receivablesByAcquirer,
        optInConfirmed: optInAuthorizations.has(client.id)
      } as ClientReceivables;
    });
  }, [importedClients, optInAuthorizations]);
  ...
  const handleManualClientAdd = (client: Client | Client[]) => {
    if (Array.isArray(client)) {
      setImportedClients(prev => [...prev, ...client]);
    } else {
      setImportedClients(prev => [...prev, client]);
    }
  };
  ...
  const handleClientSelect = (client: Client) => {
    if (!optInAuthorizations.has(client.id)) {
      return;
    }
  ...
  const handleBatchModeToggle = () => {
    if (!batchMode && importedClients.length > 0) {
      const eligibleClients = importedClients.filter(
        client => optInAuthorizations.has(client.id) && !clientsWithContracts.has(client.id)
      );
  ...
  const handleMassLockConfirm = () => {
    ...
    setConfirmedClientName(`${selectedForBatch.size} clientes`);
  ...
  const renderUploadStep = () => (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">Escolha como adicionar os clientes para recuperação de crédito</p>
      </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-3">
              ...
              {importedClients.filter(c => optInAuthorizations.has(c.id) && !clientsWithContracts.has(c.id)).length > 1 && (
                <button
                ...
                <div key={client.id} className="space-y-2">
                  <div className="flex items-start gap-3">
                    <button
                      ...
                      disabled={!hasOptInAuth && !hasContract}
                      className={`flex-1 flex items-center justify-between p-4 rounded-lg border transition-all ${
                        hasContract
                    ...
                    >
                      <div className="text-left flex-1">
                        ...
                        {!hasContract && hasOptInAuth && (
                          <p className="text-xs text-blue-600 font-medium mt-1">Autorização confirmada</p>
                        )}
                      </div>
                      <div className="flex items-center space-x-3">
                        <CheckCircle className={`w-5 h-5 ${hasContract ? 'text-green-600' : hasOptInAuth ? 'text-blue-600' : 'text-gray-400'}`} />
                        <ArrowRight className={`w-5 h-5 ${hasOptInAuth || hasContract ? 'text-gray-600' : 'text-gray-300'}`} />
                      </div>
                    </button>
                    {hasOptInAuth && !hasContract && (
                      <div className="w-64">
                        <div className="text-xs text-gray-600 mb-1 text-center">
                          Visão de recebíveis livres em 30 dias
                        </div>
                        <ReceivablesIndicator
                          total={clientsReceivables.find(r => r.clientId === client.id)?.totalReceivables || 0}
                          locked={clientsReceivables.find(r => r.clientId === client.id)?.lockedReceivables || 0}
                          available={clientsReceivables.find(r => r.clientId === client.id)?.availableReceivables || 0}
                          size="sm"
                          showLabels={false}
                        />
                      </div>
                  ...
                  {!hasContract && (
                    <div className="ml-4 pl-4 border-l-2 border-blue-200 space-y-2">
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={contractualConsents.has(client.id)}
  ...
  const renderBatchStep = () => {
    const eligibleClients = clientsReceivables.filter(client =>
      optInAuthorizations.has(client.clientId) && !clientsWithContracts.has(client.clientId)
    );
  ...
  const renderSummaryStep = () => (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">Processo de recuperação de crédito concluído com sucesso</p>
      </div>
  ...
  return (
    <>
            ...
            <div>
              ...
              <div className="flex items-center space-x-2 mt-2">
                <div className={`flex items-center space-x-2 ${currentStep === 'upload' ? 'text-blue-600' : 'text-gray-400'}`}>
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium ${currentStep === 'upload' ? 'bg-blue-600 text-white' : 'bg-gray-300 text-gray-600'}`}>
                    1
                  </div>
                  <span className="text-sm font-medium">Clientes</span>
                </div>
                <div className="w-8 h-0.5 bg-gray-300"></div>
                <div className={`flex items-center space-x-2 ${currentStep === 'radar' ? 'text-blue-600' : 'text-gray-400'}`}>
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-medium ${currentStep === 'radar' ? 'bg-blue-600 text-white' : 'bg-gray-300 text-gray-600'}`}>
                    2
                  </div>
      ...
      <MassLockConfirmation
        ...
        clients={clientsReceivables}
        lockOperations={lockOperations}
        selectedClients={selectedForBatch}
      />

      <BatchAdvancedConfigModal
        isOpen={showAdvancedConfigModal}
        onClose={() => {
          setShowAdvancedConfigModal(false);
          setCurrentConfigClientId(null);
        }}
        onSave={handleSaveAdvancedConfig}
        clientName={
          currentConfigClientId
            ? importedClients.find(c => c.id === currentConfigClientId)?.name || ''
            : ''
        }
        clientId={currentConfigClientId || ''}
        initialConfig={
          currentConfigClientId
            ? advancedConfigs.get(currentConfigClientId)
            : undefined
        }
      />
