...
import { TrendingUp, Lock, CheckCircle, Calendar, ArrowLeft, Eye, FileText, Plus, Filter, Search, Activity, ChevronDown, ChevronRight } from 'lucide-react';
                ...
                <div className="px-4 pb-4 pt-0 space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    {([
                      ...
                      { key: 'mine' as const, label: 'Bloqueado para mim' },
                      { key: 'others' as const, label: 'Bloqueado por terceiros' },
                    ]).map((opt) => (
                      <button
                        key={opt.key}
                        onClick={() => setBloqueadoFilter(opt.key)}
                        className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                          bloqueadoFilter === opt.key
                  ...
                  {(bloqueadoFilter === 'all' || bloqueadoFilter === 'mine') && (
                    <div>
                      {bloqueadoFilter === 'all' && (
                        <p className="text-xs font-semibold text-red-800 mb-1.5 uppercase tracking-wide">Bloqueado para mim</p>
                      )}
                      <div className="space-y-2">
                        <div className="flex items-center justify-between p-3 bg-red-100/50 rounded-lg">
                          ...
                          <span className="text-sm font-semibold text-red-700">{formatCurrency(Math.round(totalFutureBlocked * 0.45))}</span>
                        </div>
                        <div className="flex items-center justify-between p-3 bg-red-100/50 rounded-lg">
                          <span className="text-sm text-gray-700">Garantia</span>
                          <span className="text-sm font-semibold text-red-700">{formatCurrency(Math.round(totalFutureBlocked * 0.2))}</span>
                        </div>
                        <div className="flex items-center justify-between p-3 bg-red-100/70 rounded-lg font-medium">
                          ...
                          <span className="text-sm font-bold text-red-700">{formatCurrency(Math.round(totalFutureBlocked * 0.65))}</span>
                        </div>
                      </div>
                    </div>
                  )}
