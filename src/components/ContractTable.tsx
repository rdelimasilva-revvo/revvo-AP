  ...
  const filteredAndSortedContracts = useMemo(() => {
    const filtered = contracts.filter(contract => {
      if (simpleFilter) {
        ...
        if (productTypeFilter && contract.productType !== productTypeFilter) return false;
        return true;
      }
    ...
    return filtered;
  }, [contracts, filters, sortField, sortDirection, clientSearchTerm, clients, simpleFilter, showClosed, productTypeFilter]);
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2 flex-wrap">
              {[
                { value: '', label: 'Todas' },
                { value: 'debt-settlement', label: 'Recuperação de crédito' },
                { value: 'guarantee', label: 'Garantias' },
                { value: 'extra-limit', label: 'Pré-pagamento' },
              ...
              >
                ...
                {showClosed ? 'Ocultar encerrados' : `Encerrados (${closedCount})`}
              </button>
            </div>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap cursor-pointer" onClick={() => onContractClick(contract)}>
                    <div className="flex items-center">
                      ...
                      <div className="ml-2 text-xs text-gray-500">
                        ({((contract.encumberedValue / contract.requestedValue) * 100).toFixed(1)}%)
                      </div>
                    </div>
                  </td>
                </tr>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="font-bold text-blue-900">
                  {(
                    (filteredAndSortedContracts.reduce((sum, contract) => sum + contract.encumberedValue, 0) /
                    filteredAndSortedContracts.reduce((sum, contract) => sum + contract.requestedValue, 0)) * 100
                  ).toFixed(1)}%
                </span>
              </div>
            </div>
