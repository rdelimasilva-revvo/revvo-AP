...
export const Sidebar: React.FC<SidebarProps> = ({ activeSection, setActiveSection, collapsed, onToggleCollapse, onHelpClick: _onHelpClick }) => {
  ...
  const operationsSubItems = [
    { id: 'contracts', label: 'Todos os contratos' },
    { id: 'contract-approval', label: 'Aprovação de contratos' },
  ];
  ...
  const configSubItems = [
    ...
    { id: 'settlement-accounts', label: 'Contas de Liquidação' },
    { id: 'operation-parameters', label: 'Parâmetros de Operação' },
  ...
  // Mini-icon items for collapsed sidebar
  const collapsedMenuItems: NavItem[] = [
    ...
    { id: 'disputes', label: 'Contestação', icon: ShieldAlert },
    { id: 'control-panel', label: 'Painel de Controle', icon: ClipboardCheck },
  ];

  const collapsedConfigItems: NavItem[] = [
    { id: 'financial', label: 'Configurações', icon: Settings, subItems: configSubItems },
    { id: 'cnab-generator', label: 'CNAB', icon: FileCode, subItems: cnabSubItems },
  ];

  const collapsedOtherItems: NavItem[] = [
    { id: 'file-processing', label: 'Monitória', icon: BarChart3, subItems: monitoriaSubItems },
    ...
    return (
      <div className="bg-white border-r border-gray-100 h-screen w-16 fixed left-0 top-0 z-30 flex-col hidden lg:flex">
        ...
        <nav className="flex-1 overflow-y-auto py-4 flex flex-col items-center space-y-1">
          {collapsedMenuItems.map(renderCollapsedItem)}
          ...
          {collapsedOtherItems.map(renderCollapsedItem)}
        </nav>
      ...
      <div className="bg-white border-r border-gray-100 h-screen w-[280px] sm:w-64 fixed left-0 top-0 z-30 flex flex-col">
        ...
        <nav className="flex-1 overflow-y-auto px-4 pt-6">
          ...
          <div className="space-y-1">
            ...
            {renderSimple('disputes', ShieldAlert, 'Contestação')}
            {renderSimple('control-panel', ClipboardCheck, 'Painel de Controle')}
          </div>
          ...
          <div className="space-y-1">
            {renderExpandable('config-group', Settings, 'Configurações', configExpanded, setConfigExpanded, configSubItems)}
            {renderExpandable('cnab-group', FileCode, 'CNAB', cnabExpanded, setCnabExpanded, cnabSubItems)}
          </div>
          ...
          <div className="space-y-1">
            {renderExpandable('monitoria-group', BarChart3, 'Monitória', monitoriaExpanded, setMonitoriaExpanded, monitoriaSubItems)}
